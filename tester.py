...
def test(target_name, grammar='kaguya0.tpeg', print_log='True'):
    ...
    for count, s in enumerate(input_list):
        ...
        if 'Syntax Error' in repr(tree):
            results.append((False, count+1, s, str(tree)))
            fail_cnt += 1
        else:
...
def test_with_graph(target_file, grammar_file, print_log='True'):
    if not shutil.which('dot'):
        print('Not find "dot" command')
        print('Please install "Graphviz"')
        sys.exit()
    ...
    succs = list(filter(lambda l: l[0], results))
    MAX_COUNT = len(succs)
    make_dir(f'graph/{FILE_NAME}')
    for count, (bl, ln, s, ast) in enumerate(succs):
        sys.stdout.write(f'\rNow Processing: {count+1}/{MAX_COUNT}')
        gen_graph(escape(s), ast, f'graph/{FILE_NAME}/{ln}.png')
        sys.stdout.flush()
    print()
